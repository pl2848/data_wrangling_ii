...
str_detect(string_vec,"[Bb]ush") # [] means case not sensitive
```
...
str_detect(string_vec, "^[0-9][a-zA-Z]") #means a number and a letter
```
...
str_detect(string_vec, "7.11") # the dot means anything
```
...
data_marj = 
  ...
  separate(age_year, into = c("age", "year"), sep = "\\(") %>% 
    mutate(
    year = str_replace(year, "\\)", ""), # str_replace, can te
    percent = str_replace(percent, "[a-c]$", ""),
    percent = as.numeric(percent)) %>% 
  filter(!(State %in% c("Total U.S.", "Northeast", "Midwest", "South", "West"))) # do the reverse. Thake false to make it true 
...
data_marj = 
  table_marj %>%
  select(-contains("P Value")) %>% # get rid of things that contain P value
  pivot_longer(
    -State,
    names_to = "age_year", 
    values_to = "percent") %>%
